<template>
  ...
  <div>
    ...
    <svg id="svg" width="1200" height="750">
...
export default {
  ...
  data () {
    return {
      ...
      colors: ['#801224', '#aaaaff', '#4e88af', '#968d8f','#FFC0CB', '#BA55D3', '#1E90FF', '#7FFFD4','#FFFF00'],
      states: [],
  ...
  methods: {
    ...
    d3render () {
      ...
      var node = svg.append("g")
        ...
        .attr("fill", d => {
          if ((d.properties.name).indexOf('.jpg') != -1){            
            //      <pattern id="image" patternUnits="userSpaceOnUse" height="100" width="100">
            //       <image x="0" y="0" height="100" width="100" xlink:href="/root/jzb/web/img/tank_1.jpg"></image>
            //       </pattern> 
            var rec = svg.append("defs")
              rec.append("pattern")
                .attr("class","img"+(d.properties.name).split('.')[0])
                .attr("id","image"+(d.properties.name).split('.')[0])
                .attr("patternUnits","userSpaceOnUse")
                .attr("height","100")
                .attr("width","100")
                .selectAll("img"+(d.properties.name).split('.')[0])
                .data("1").enter()
                .append("image")
                .attr("x", "0")
                .attr("y", "0")
                .attr("height","100")
                .attr("width","100")
                .attr("xlink:href","http://localhost:8888/static/"+d.properties.name)
            return "url(#image"+(d.properties.name).split('.')[0]+")"
          }else{
            (this.nodes_name).push(d)
      ...
      console.log(this.nodes_name)
      var text = svg.append("g")
        .attr("class", "texts")
        .selectAll("text")
        .data(this.nodes_name).enter()
